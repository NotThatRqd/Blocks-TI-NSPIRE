...
uint8_t world::compute_top_shadow(uint8_t x, uint8_t y, uint8_t z) {
    ...
    if (tri_grid_shadow[tri_grid_idx] < depth) shadow |= SHADOW_TOP;
    tri_grid_idx++;
    if (tri_grid_shadow[tri_grid_idx] < depth) shadow |= SHADOW_BOTTOM;
    return shadow;
}
...
void world::set_water(int x, int y, int z) {
    ...
    if ((y == WORLD_HEIGHT - 1) || (blocks[y + 1][x][z] != WATER)) {
        if ((z == WORLD_SIZE - 1) || (blocks[y][x][z + 1] != WATER)) water_left[MID_FACE] = WATER_HALF;
        if ((x == WORLD_SIZE - 1) || (blocks[y][x + 1][z] != WATER)) water_right[MID_FACE] = WATER_HALF;
...
void world::set_block_shadow(int x, int y, int z) {
    ...
    for (uint8_t s = 0; s < 3; s++) {
        ...
        if (tri_grid_shadow[tri_grid_idx] > depth) tri_grid_shadow[tri_grid_idx] = depth;
        tri_grid_idx++;
        if (tri_grid_shadow[tri_grid_idx] > depth) tri_grid_shadow[tri_grid_idx] = depth;
    }
}
...
void world::place_block(int x, int y, int z, Block_t block) {
    ...
    for (uint8_t i = 0; i < 6; i++) refresh_shadows(x_update[i], y_update[i], z_update[i]);
}
...
void world::clear_world() { fill_space(0, 0, 0, WORLD_SIZE - 1, WORLD_HEIGHT - 1, WORLD_SIZE - 1, AIR); }